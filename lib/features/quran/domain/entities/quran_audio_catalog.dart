import 'quran_audio_option_entity.dart';

export 'quran_audio_option_entity.dart' show QuranAudioOption;

/// The 28-reciter Quran audio catalog, streamed from `cdn.islamic.network`
/// by `QuranAudioBloc` (see `presentation/bloc/quran_audio_bloc/`).
///
/// Moved here verbatim from
/// `lib/src/features/quran/audio/quran_audio_catalog.dart` - zero logic
/// changes. Judgment call (see `PROGRESS.md` TASK-024): this is a genuinely
/// static, hardcoded, zero-I/O list (no network/DB/asset read to populate
/// it - the actual network I/O happens later, per-request, inside
/// `QuranAudioBloc` when it builds a stream URL from a chosen id). That
/// makes it pure domain-level reference data rather than a "data source" in
/// the Clean Architecture sense, so it stays under `domain/entities/`
/// alongside the entity it catalogs rather than under
/// `data/datasources/`.
